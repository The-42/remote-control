...
struct netcb_data {
	struct net_udp_channel *chan;
};

static void recv_callback(struct net_udp_channel *chan, void *data)
{
	struct netcb_data *ncbd = data;

	g_assert_nonnull(ncbd);
	g_assert_nonnull(chan);
...
int main(int argc, char **argv)
{
	...
	struct netcb_data netcb_data = { NULL };
	...
	g_usleep(300000);
	g_assert_true(netcb_data.chan == secondary);
	...
	g_usleep(300000);
	g_assert_true(netcb_data.chan == primary);
