...
struct panel_data {
	...
	GtkComboBox *combo;
	GtkHScale *volume;
	GtkCheckButton *muted;
	/* the currently select mixer control */
	enum medcom_mixer_control mixer;
};

struct mixer_control_mapping {
	enum medcom_mixer_control id;
	const gchar* text;
};

static const struct mixer_control_mapping mixer_map[] = {
...
static void audio_panel_dump_priv(struct panel_data *priv)
{
	...
	fprintf(stdout, "    volume:   %p\r\n", priv->volume);
	fprintf(stdout, "    muted:    %p\r\n", priv->muted);
	fprintf(stdout, "   mixer:     %d\r\n", priv->mixer);
}

static int audio_panel_update_controls(struct panel_data *pnl)
{
	uint8_t volume;
	bool muted;
	int ret;
	...
	ret = medcom_mixer_get_volume(pnl->client, pnl->mixer, &volume);
	if (ret < 0) {
		printf(" medcom_mixer_get_volume(%p, %d, ...) failed with %d\n",
			pnl->client, pnl->mixer, ret);
		return ret;
	}

	gtk_range_set_value(GTK_RANGE(pnl->volume), (gdouble)volume);

	ret = medcom_mixer_get_mute(pnl->client, pnl->mixer, &muted);
	if (ret < 0) {
		printf(" medcom_mixer_get_mute(%p, %d, ...) failed with %d\n",
			pnl->client, pnl->mixer, ret);
		return ret;
	}
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(pnl->muted), muted);

	return 0;
}

void on_channel_select_changed(GtkComboBox *combo, gpointer data)
{
	...
	if (string) {
		pnl->mixer = mixer_name_to_id(string);
		audio_panel_update_controls(pnl);
...
void on_channel_volume_value_changed(GtkWidget *widget, gpointer data)
{
	struct panel_data *pnl;
	gdouble val;
	int32_t ret;
	...
	ret = medcom_mixer_set_volume(pnl->client, pnl->mixer, (uint8_t)val);
	if (ret < 0) {
...
void on_channel_mute_toggled(GtkWidget *widget, gpointer data)
{
	struct panel_data *pnl;
	gboolean mute;
	int32_t ret;
	...
	ret = medcom_mixer_set_mute(pnl->client, pnl->mixer, mute);
	if (ret < 0) {
...
static int audio_panel_fill_combo_box(GtkComboBox *combo)
{
	GtkTreeModel *model;
	...
	if (!model) {
		...
		for (i=0; i<G_N_ELEMENTS(mixer_map); i++) {
			...
			gtk_list_store_set(list, &iter, 0, mixer_map[i].text, -1);
		}
...
static int audio_panel_create(struct panel *panel, GtkWidget **widget)
{
	...
	priv->mixer = MEDCOM_MIXER_CONTROL_PLAYBACK_MASTER;

	priv->combo = GTK_COMBO_BOX(glade_xml_get_widget(xml, "channel_select"));
	if (priv->combo)
		g_object_set_data(G_OBJECT(priv->combo), USER_DATA_KEY, priv);

	priv->volume = GTK_HSCALE(glade_xml_get_widget(xml, "channel_volume"));
	if (priv->volume)
		g_object_set_data(G_OBJECT(priv->volume), USER_DATA_KEY, priv);

	priv->muted  = GTK_CHECK_BUTTON(glade_xml_get_widget(xml, "channel_mute"));
	if (priv->muted)
		g_object_set_data(G_OBJECT(priv->muted), USER_DATA_KEY, priv);
	...
	audio_panel_dump_priv(priv);
	/* fill the combo box only, when we have processed all widgets */
	if (priv->combo)
		audio_panel_fill_combo_box(priv->combo);
